...
const newUser = TryCatch(async (req, res, next) => {
  ...
  const user = await User.create({
    ...
    password,
    avatar,
    language: language || "en", // ✅ Store language (fallback to English)
  });
...
const logout = TryCatch(async (req, res) => {
  return res
    .status(200)
    .cookie("Globe-token", "", { ...cookieOptions, maxAge: 0 })
    .json({
      success: true,
...
const searchUser = TryCatch(async (req, res) => {
  ...
  // Find all chats this user is part of (non-group)
  const myChats = await Chat.find({ groupChat: false, members: req.user });

  // All people I've already chatted with
  const allUsersFromMyChats = myChats.flatMap((chat) => chat.members);

  // Exclude: myself + people I've already chatted with
